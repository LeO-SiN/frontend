...
def create_post(post: NotesSchema, user_data=Depends(jwt.get_current_user)):
    try:
        posts_handler = NotesHandler()
...
def update_post(id: str, post: NotesSchema, user_data=Depends(jwt.get_current_user)):
    try:
        posts_handler = NotesHandler()
